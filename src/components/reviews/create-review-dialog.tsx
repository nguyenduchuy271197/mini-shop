...
export function CreateReviewDialog({
  ...
  const createReview = useCreateReview({
    onSuccess: () => {
      toast({
        title: "Thành công",
        description:
          "Đã gửi đánh giá thành công. Đánh giá sẽ được hiển thị sau khi được duyệt.",
      });
      setOpen(false);
      resetForm();
      onSuccess?.();
  ...
  const defaultTrigger = <Button>Viết đánh giá</Button>;
      ...
      <DialogContent className="sm:max-w-[500px]">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="bg-gray-50 p-4 rounded-lg">
            ...
            {orderId && (
              <p className="text-sm text-gray-600 mt-1">Đơn hàng #{orderId}</p>
            )}
          </div>
